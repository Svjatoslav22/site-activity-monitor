...
export default function App() {
  ...
  // prevent unused-function TS6133 while iterating UI
  ...
  const handleAiAnalysis = async (site: Site) => {
    ...
    const prompt = `Проаналізуй моніторинг сайту:
- Назва: ${site.name}
- URL: ${site.url}
...
Напиши міні-звіт українською: 3-4 речення про стан і 1-2 рекомендації. Без англійських слів.`;
    const systemPrompt =
      'Ти SRE-інженер. Відповідай лише українською, коротко і формально.';
  ...
  const handleGlobalAnalysis = async () => {
    ...
    const sitesData = sites
      .map(
        (site) =>
          `- ${site.name} (${site.url}): статус ${site.status === 'active' ? 'активний' : 'неактивний'}, пінг ${site.ping} мс, аптайм ${site.uptime}%`,
      )
      .join('\n');

    const prompt = `Дані моніторингу:\n${sitesData}

Напиши короткий звіт для керівництва українською мовою у такому форматі:
1. Загальний стан — 1-2 речення
2. Критичні проблеми — лише сайти зі статусом «неактивний» або пінгом понад 500 мс; якщо таких немає, напиши «критичних проблем немає»
3. Рекомендації на сьогодні — 2-3 короткі пункти

Не використовуй англійську, латиницю-мішанину чи випадкові слова.`;
    const systemPrompt =
      'Ти технічний директор. Відповідай лише українською. Короткий структурований звіт, без води.';
    ...
    try {
      const res = await apiFetch('/api/ai/generate', {
        ...
        body: JSON.stringify({
          prompt,
          systemPrompt:
            "Ти PR-фахівець в IT. Пиши лише українською, 4-6 речень, без мішанини мов.",
        }),
      });
      if (!res.ok) throw new Error('AI error');
      const data = await res.json();
